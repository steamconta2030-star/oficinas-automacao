  ...
  return (
    <>
      ...
      {ordens.length === 0 ? (
        ...
        <section className="os-list">{ordens.map((os) => <Link key={os.id} to="/os/$osId" params={{ osId: os.id }} className="os-row"><div><strong>OS #{os.numero}</strong><span>{os.cliente.nome} · {os.item.identificacao}</span></div><span className="status-chip">{ordemServicoStatusLabel[os.status]}</span></Link>)}</section>
      )}
    </>
  )
}
